...
\newline
\newline
Para $i$ desde 1 hasta $n-1$ hacer, \TipoVariable{Insertar(i)}
\newline
\newline
Para $i$ desde 1 hasta $n-1$ hacer
\newline
\newline
\TipoVariable{j $\leftarrow$ i-1, elem $\leftarrow$ a[i]}
\newline
\newline
\TipoVariable{mientras j $\geq$ 0 $\yluego$ a[j] $>$ elem} hacer
\newline
\newline
\TipoVariable{a[j+1] $\leftarrow$ a[j]} 
\newline
\newline
\TipoVariable{j $\leftarrow$ j-1} 
\newline
\newline
\TipoVariable{endwhile} 
\newline
\newline
\TipoVariable{a[j+1] $\leftarrow$ elem} 
\newpage
...
\newline
\newline
\TipoVariable{heap $\leftarrow$ arrayToMaxHeap($A$)}
\newline
\newline
\TipoVariable{para $i$ desde $n-1$ hasta $0$ hacer}
\newline
\newline
\TipoVariable{max $\leftarrow$ proximo(heap)}
\newline
\newline
\TipoVariable{desencolar}
\newline
\newline
\TipoVariable{$A[i]$ $\leftarrow$ max}
\newline
\newline
Por lo tanto, el costo de este algoritmo es $O(n)$ para la operación \TipoVariable{arrayToMaxHeap($A$)} y $O(n~log(n))$ para el ciclo principal ya que la operación más costosa es desencolar que tiene costo $O(log(n))$ y como tenemos $n$ elementos en el arreglo, el costo final del ciclo es $O(n~log(n))$. Concluimos que el costo del algoritmo es $O(n)$ + $O(n~log(n))$ = $O(max\{n,n~log(n)\})$ = $O(n~log(n))$. 
\newline
\newline
...
\newline
\newline
1. Dividir el arreglo en dos partes iguales, o sea, por la mitad
\newline
\newline
2. Ordenar recusivamente (a través del mismo algoritmo) ambas mitades
\newline
\newline
3. Fundir ambas mitades (ya ordenadas) en un único arreglo
\newline
\newline
Veamos el siguiente ejemplo:
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{merge-sort-1}
	\label{drivers1}
\end{figure}
\newpage
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{merge-sort-2}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\newline
\newline
\TipoVariable{MERGE-SORT $A[0,..., n-1]$} con costo $T(n)$ definido para poder establecer el costo de la recursión
\newline
\newline
1. Si $n=1$, ya está ordenado con costo $O(1)$
\newline
\newline
2. Recursivamente ordenar \TipoVariable{A[0,..., $\displaystyle \frac{n}{2} - 1$]} y \TipoVariable{A[$\displaystyle \frac{n}{2} - 1$, $n-1$]} con costo $2T(\displaystyle \frac{n}{2})$
\newline
\newline
3. $Merge$ las dos mitades ordenadas con costo $O(n-1)$ 
\newline
\newline
Es decir:
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.3\textwidth]{quick-sort-1}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.4\textwidth]{quick-sort-2}
	\label{drivers1}
\end{figure}
\newline
\newline
\newline
En la figura \TipoVariable{a)}, colocamos el arreglo que queremos ordenar.
\newline
\newline
En la figura \TipoVariable{b)}, primero buscamos el máximo (12) del arreglo y lo colocamos al final (costo $O(n)$) intercambiandolo con el elemento que se encuentra en la última posición. Luego, elegimos como \textit{pivot} al elemento central (6) del arreglo, es decir, al elemento que se encuentra en el medio y lo colocamos al inicio intercambiandolo con el elemento que se encuentra en la primera posición. Por último, seteamos como \textit{lower} al segundo elemento del arreglo (5) y como \textit{upper} al $n-1$ elemento del arreglo (10). 
\newline
\newline
En la figura \TipoVariable{c)}, implementamos la siguiente estrategia. Creamos dos ciclos donde en uno comparamos el valor actual de \textit{lower} con el del \textit{pivot} y como 5 $<$ 6, avanzo \textit{lower}. Mismo caso para el 4 y como 7 $<$ 6 $Abs!$, detengo a \textit{lower} apuntando al 7. Ahora, el caso de \textit{upper} es análogamente inverso, es decir, si elemento que apunta \textit{upper} es mayor al \textit{pivot}, avanzo \textit{upper} hacia la izquierda, por eso, \textit{upper} se detiene en 3.   
\newline
\newline
En la figura \TipoVariable{d)}, como tengo un elemento mayor al \textit{pivot} y otro menor, los intercambio de posición.
\newline
\newline
En la figura \TipoVariable{e)}, avanzo \textit{upper} y \textit{lower} con el mismo criterio que antes y por lo tanto los detengo cuando apuntan al 8 y 6 respectivamente.
\newline
\newline
En la figura \TipoVariable{f)}, realizo el intercambio entre estos elementos.
\newline
\newline
En la figura \TipoVariable{g)}, ambos punteros se cruzan porque 6 es menor/igual que 6 y 8 $>$ 6.
\newline
\newline
En la figura \TipoVariable{h)}, \textit{upper} apunta a 1 porque 1 no es mayor que 6 y \textit{lower} apunta a 1 porque 8 no es menor que 6. Cuando quedan cruzados, como en este caso,  significa que termine con esta iteración.
\newline
\newline
En la figura \TipoVariable{i)}, como termine la iteración, intercambio al elemento que apunta \textit{upper} (1) con el \textit{pivot} que se habia elegido inicialmente (6) obteniendo el resultado de la figura.
\newline
\newline
En la figura \TipoVariable{j)}, aplico recursivamente el mismo algoritmo a las dos nuevas mitades.
\newpage
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.4\textwidth]{quick-sort-3}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.4\textwidth]{quick-sort-4}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{dec-1}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.8\textwidth]{dec-2}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\newline
\newline
1. Descomponer $X$ en subinstancias $X_{1}$,$X_{2}$,...,$X_{n}$
\newline
\newline
2. Para $i$ desde 1 hasta $k$ hacer $Y_{i} \leftarrow$ DC($X_{i}$)
\newline
\newline
3. Combinar las soluciones $Y_{i}$ para construir una solución $Y$ para $X$.
...
\newline
\newline
1. Cuando $a < c$, o sea, pocos subproblemas $\displaystyle \frac{a}{c} < 1$ y por lo tanto, la serie converge lo que implica que:
\newline
\newline
...
\newline
\newline
2. Cuando $a = c$, por ejemplo, en el caso del Merge Sort lo que implica que:
\newline
\newline
...
\newline
\newline
3. Cuando $a > c$, o sea, muchos subproblemas, lo que implica que:
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{puntos-1}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{puntos-2}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{puntos-3}
	\label{drivers1}
\end{figure}
\newpage
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{puntos-4}
	\label{drivers1}
\end{figure}
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{puntos-5}
	\label{drivers1}
\end{figure}
\newpage
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{puntos-6}
	\label{drivers1}
\end{figure}
\newline
\newline
...
Supongamos que tenemos que ordenar los registros con valores \TipoVariable{EJEMPLODEORDENACIONFUSION}.
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{fme-1}
	\label{drivers1}
\end{figure}
\newpage
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{fme-2}
	\label{drivers1}
\end{figure}
\newline
\newline
Es decir, en el primer registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{E}, \TipoVariable{L} y \TipoVariable{D} el cual resulta ser \TipoVariable{D} 
\newline
\newline
Avanzo el cabezal de la cinta 3 y en el segundo registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{E}, \TipoVariable{L} y \TipoVariable{E} el cual resulta ser \TipoVariable{E} (elijo el que se encuentra ubicado en la cinta 3).
\newline
\newline
Avanzo el cabezal de la cinta 3 y en el tercer registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{E}, \TipoVariable{L} y \TipoVariable{E} el cual resulta ser \TipoVariable{E}
\newline
\newline
Avanzo el cabezal de la cinta 3 y en el cuarto registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{E}, \TipoVariable{L} y \TipoVariable{O} el cual resulta ser \TipoVariable{E}
\newline
\newline
Avanzo el cabezal de la cinta 1 y en el quinto registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{J}, \TipoVariable{L} y \TipoVariable{O} el cual resulta ser \TipoVariable{J}
\newline
\newline
Avanzo el cabezal de la cinta 1 y en el quinto registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{J}, \TipoVariable{L} y \TipoVariable{O} el cual resulta ser \TipoVariable{J}
\newline
\newline
El cabezal de la cinta 1 no apunta a más registros y en el sexto registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{L} y \TipoVariable{O} el cual resulta ser \TipoVariable{L}
\newline
\newline
Avanzo el cabezal de la cinta 2 y en el septimo registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{M} y \TipoVariable{O} el cual resulta ser \TipoVariable{M}
\newline
\newline
Avanzo el cabezal de la cinta 2 y en el octavo registro de la cinta 4 colocamos el registro de menor valor entre \TipoVariable{P} y \TipoVariable{O} el cual resulta ser \TipoVariable{P}
\newline
\newline
El cabezal de la cinta 3 no apunta a más registros y en el noveno registro de la cinta 4 colocamos al registro \TipoVariable{P}
\newline
\newline
Por lo tanto, la cinta 4 queda ordenada de la siguiente manera: \TipoVariable{DEEEJLMOP}.
\newline
\newline
Esto mismo lo repito para los bloques 2 y 3 colocandolos en la cintas 4 y 6 respectivamente.
\newline
\newline
Finalmente, realizando una última pasada de la misma manera que venimos haciendo pero considerando ahora las cintas 4,5 y 6, logramos ordenar los registros de \TipoVariable{EJEMPLODEORDENACIONFUSION}. 
\newpage
...
\newline
\newline
Analicemos el siguiente ejemplo: \TipoVariable{EJEMPLODEORDENACIONFUSION}. 
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-1}
	\label{drivers1}
\end{figure}
\newline
\newline
Tenemos la cola de prioridad de tamaño $M$ = 3 registros y la $salida$ donde vamos a ir colocando los bloques ordenados. Entonces, llenamos la cola de prioridad con los registros \TipoVariable{E}, \TipoVariable{J} y \TipoVariable{E} de manera que al encolarse quedan ordenados.
\newline
\newline
Luego, desencolo la raíz (\TipoVariable{E}) y coloco a este registro en alguna de la cintas de salida como se muestra a continuación:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-2}
	\label{drivers1}
\end{figure}
\newpage

Como saqué al registro \TipoVariable{E}, se libera un lugar para que pueda encolarse el registro \TipoVariable{M}. Luego, coloco al registro \TipoVariable{E} en alguna de la cintas de salida como se muestra a continuación:  
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-3}
	\label{drivers1}
\end{figure}
\newline
\newline
Como saqué al registro \TipoVariable{E}, se libera un lugar para que pueda encolarse el registro \TipoVariable{P}. Luego, coloco al registro \TipoVariable{J} en alguna de la cintas de salida como se muestra a continuación:  
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-4}
	\label{drivers1}
\end{figure}
\newline
\newline
Como saqué al registro \TipoVariable{J}, se libera un lugar para que pueda encolarse el registro \TipoVariable{L}. Luego, coloco al registro \TipoVariable{L} en alguna de la cintas de salida como se muestra a continuación:  
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-5}
	\label{drivers1}
\end{figure}
\newpage

Como saqué al registro \TipoVariable{L}, se libera un lugar para que pueda encolarse el registro \TipoVariable{O}. Luego, coloco al registro \TipoVariable{M} en alguna de la cintas de salida como se muestra a continuación:  
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-6}
	\label{drivers1}
\end{figure}
\newline
\newline
Pero al escribir el registro \TipoVariable{M}, ingresa el registro \TipoVariable{D} que es menor al registro \TipoVariable{M} que acaba de salir, lo marco a \TipoVariable{D} porque quiero generar salidas lo más largas posibles ordenadas. De lo contrario, el bloque de registros que estoy mostrando por la salida dejaria de estar ordenado \TipoVariable{E-E-J-L-M-O-D} $Abs!$. 
\newline
\newline
Por lo tanto, coloco al registro \TipoVariable{O} en alguna de la cintas de salida como se muestra a continuación:  
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-7}
	\label{drivers1}
\end{figure}      
\newline
\newline
Nuevamente, al escribir el registro \TipoVariable{O}, ingresa el registro \TipoVariable{E} que es menor al registro \TipoVariable{O} que acaba de salir, lo marco a \TipoVariable{E} como si fuera mayor porque quiero generar salidas lo más largas posibles ordenadas y coloco al registro \TipoVariable{P} en alguna de la cintas de salida como se muestra a continuación:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-8}
	\label{drivers1}
\end{figure}      
\newpage

Al desencolar el registro \TipoVariable{P}, ingresa el registro \TipoVariable{O} que es menor al registro \TipoVariable{P} que acaba de salir. Entonces, comienzo un nuevo bloque y la raíz pasa a ser el registro \TipoVariable{D} y lo coloco en alguna de la cintas de salida como se muestra a continuación:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-9}
	\label{drivers1}
\end{figure}      
\newline
\newline
Como saqué al registro \TipoVariable{D}, se libera un lugar para que pueda encolarse el registro \TipoVariable{R}. Luego, coloco al registro \TipoVariable{E} en alguna de la cintas de salida como se muestra a continuación:  
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-10}
	\label{drivers1}
\end{figure}
\newline
\newline
Al desencolar el registro \TipoVariable{E}, ingresa el registro \TipoVariable{D} que es menor al registro \TipoVariable{E} que acaba de salir, lo marco a \TipoVariable{D} como mayor y coloco al registro \TipoVariable{O} en alguna de la cintas de salida como se muestra a continuación:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-11}
	\label{drivers1}
\end{figure}      
\newpage

Nuevamente, al escribir el registro \TipoVariable{O}, ingresa el registro \TipoVariable{E} que es menor al registro \TipoVariable{O} que acaba de salir, lo marco a \TipoVariable{E} como si fuera mayor y coloco al registro \TipoVariable{R} en alguna de la cintas de salida como se muestra a continuación:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-12}
	\label{drivers1}
\end{figure}      
\newline
\newline
Nuevamente, estamos en condiciones de comenzar un nuevo bloque porque el registro \TipoVariable{N} es menor que el registro \TipoVariable{R} que acaba de salir como se muestra a continuación:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{susti-13}
	\label{drivers1}
\end{figure}      
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{poli-1}
	\label{drivers1}
\end{figure}      
...
Caso de acceso secuencial puro. Por ejemplo, \TipoVariable{EJEMPLODEBUSQUEDAEXTERNA} donde tenemos 2 discos con 3 páginas de 4 registros cada una como se muestra a continuación:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{isam-1}
	\label{drivers1}
\end{figure}      
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{isam-2}
	\label{drivers1}
\end{figure}      
\newline
\newline
...
\newline
\newline
\textbf{.} En la primer posicíon del disco 1 se indica que no hay un registro del último bloque de un disco anterior. En la segunda posición se indica que el último registro del primer bloque es el \TipoVariable{D} y, en la tercer posición se indica que el último registro del segundo bloque es el \TipoVariable{E}.
\newline
\newline
\textbf{.} En la primer posicíon del disco 2 se indica que el último registro del disco anterior es el \TipoVariable{E}. En la segunda posición se indica que el último registro del primer bloque es el \TipoVariable{J} y, en la tercer posición se indica que el último registro del segundo bloque es el \TipoVariable{O}.
\newline
\newline
\textbf{.} En la primer posicíon del disco 3 se indica que el último registro del disco anterior es el \TipoVariable{O}. En la segunda posición se indica que el último registro del primer bloque es el \TipoVariable{S} y, en la tercer posición se indica que el último registro del segundo bloque es el \TipoVariable{X}.
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{a-234-1}
	\label{drivers1}
\end{figure}      
\newline
\newline
Veamos el siguiente ejemplo donde las claves son \TipoVariable{EJEMPLODEBU}:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{a-234-2}
	\label{drivers1}
\end{figure}      
\newline
\newline
Ahora, queremos insertar la clave \TipoVariable{S}.
\newline
\newline
Como $S$ $>$ $M$, tendriamos que insertarlo en la rama de más a la derecha. Por lo tanto, podriamos insertar a \TipoVariable{S} en el nodo que contiene a \TipoVariable{O P U} pero como ese nodo sólo soporta 3 claves, podemos insertarlo allí pero debemos subir a la raíz a la clave \TipoVariable{P} y crear un nuevo nodo que contenga a la clave para manterer el árbol balanceado como se muestra a continuación:
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{a-234-3}
	\label{drivers1}
\end{figure}      
\newpage
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{a-234-4}
	\label{drivers1}
\end{figure}      
\newline
\newline
Finalmente, veamos como queda la secuencia de inserciones restantes, es decir, la secuencia de inserciones \TipoVariable{QUEDA}.
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{a-234-5}
	\label{drivers1}
\end{figure}      
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{hed-1}
	\label{drivers1}
\end{figure}      
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{sl-1}
	\label{drivers1}
\end{figure}      
\newline
\newline
...
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{sl-2}
	\label{drivers1}
\end{figure}      
\newline
\newline
...
\newline
\newline
A continuación, explicamos el mecanismo de \textit{splaying} que tiene 4 casos fundamentales:
\newline
\newline
\textbf{.} Si accedemos a la raíz del árbol, no hacemos nada.
\newline
\newline
\textbf{.} Si accedemos a $k$, y el padre de $k$ es la raíz, hacemos una rotación simple.
\newline
\newline
\textbf{.} Si accedemos a $k$, y el padre de $k$ no es la raíz, hay dos casos posibles (y sus especulares): rotación zig-zag y rotación zig-zig.
\newline
\newline
Como efecto del splaying no sólo se mueve el nodo accesado hacia la raíz sino que todos los nodos del camaino desde la raíz hasta el nodo accesado se mueven aproximadamente a la mitad de su profundidad anterior, a costa de que algunos pocos nodos bajen como máximo dos niveles en el árbol.
\newpage

\subsubsection{Rotación Zig-zag}

Ocurre cuando el padre del nodo $k$, que es el que estoy buscando, no es la raíz y $k$ es hijo derecho mientras que su padre $p$ es hijo izquierdo del nodo $a$ o viceversa.
\newline
\newline
Por lo tanto, comenzamos con una rotación a izquierda aplicada a $k$ que lo ubica en la posición de $p$ y luego una rotación a derecha aplicada a $k$ que lo posiciona en la raíz del árbol. 
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{st-1}
	\label{drivers1}
\end{figure}    

\subsubsection{Rotación Zig-zig}

Ocurre cuando el padre del nodo $k$, que es el que estoy buscando, no es la raíz y $k$ es hijo izquierdo mientras que su padre $p$ también es hijo izquierdo del nodo $a$ o $k$ es hijo derecho mientras que su padre $p$ también es hijo derecho del nodo $a$.
\newline
\newline
Por lo tanto, comenzamos con una rotación a derecha aplicada a $p$ y luego una rotación a derecha aplicada a $k$ que lo posiciona en la raíz del árbol. 
\newline
\newline
\begin{figure}[h]
	\centering
\includegraphics[width=0.5\textwidth]{st-2}
	\label{drivers1}
\end{figure}      

\subsubsection{Inserción y Borrado}

Para insertar, hacemos como siempre: buscamos el lugar donde corresponde insertar y efectuamos las rotaciones resultantes de esa búsqueda. El elemento insertado queda entonces en la raíz.
\newline
\newline
Para borrar un elemento de un splay tree se puede proceder de la siguiente forma: se realiza una búsqueda del nodo a eliminar, lo cual lo deja en la raíz del árbol. Si ésta es eliminada, se obtienen dos subárboles $S_{izq}$ y $S_{der}$. Se busca el elemento mayor en $S_{izq}$, $m$, que pasa a ser su nueva raíz. Como $m$ era el elemento mayor, no tenia hijo derecho, por lo que se cuelga $S_{der}$ como subárbol derecho y $S_{izq}-m$ como subárbol izquierdo con lo que termina la operación de eliminación.  
